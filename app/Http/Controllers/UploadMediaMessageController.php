...
class UploadMediaMessageController extends Controller
{
	...
	protected function uploadMusic($music)
	{
		...
		if (config('filesystems.default') == 's3') {
			// Move file to S3 in the specified path
			$this->moveAudioToS3($music);
		} else {
			// Move file to default storage
	...
	/**
	 * Move audio file to S3 in the specified path
	 *
	 * @param string $file
	 * @return void
	 */
	protected function moveAudioToS3($file)
	{
		$localFile = public_path('temp/' . $file);

		// Move the file to S3 in the specified path
		// Using the 's3' disk directly to ensure it goes to S3 regardless of default disk
		Storage::disk('s3')->putFileAs('uploads/audio', new File($localFile), $file);
