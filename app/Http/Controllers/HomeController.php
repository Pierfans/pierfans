...
class HomeController extends Controller
{
  ...
  public function index()
  {
    ...
    if (auth()->guest()) {
      ...
      $highlights = \App\Models\InitialPost::with(['updateRelation.user', 'updateRelation.media'])
        ->orderBy('order')
        ->limit(5)
        ->get()
        ->map(function ($initialPost) {
          $update = $initialPost->updateRelation;
          if ($update && $update->user) {
            $media = $update->media->first();
            $imageUrl = $media ? url($media->image) : null;
            return [
              'avatar' => url($update->user->avatar),
              'username' => $update->user->username,
              'name' => $update->user->name,
              'last_update' => [
                'description' => $update->description,
                'created_at' => $update->created_at ?? $update->date,
                'image_url' => $imageUrl,
              ],
            ];
          }
      ...
      return view('index.' . $home, [
        ...
        'highlights' => $highlights->filter(fn($h) => !empty($h['last_update']))->values(),
      ]);
    } else {
