               ...
               @if ($data->total() !=  0 && $data->count() != 0)
                 ...
                 @foreach ($data as $user)
                   <tr>
                     ...
                     <td data-label="{{ __('auth.full_name') }}" title="{{$user->name}}">
                     ...
                     <td data-label="{{ __('general.balance') }}">{{ Helper::amountFormatDecimal($user->balance)}}</td>
                     <td data-label="{{ __('general.wallet') }}">{{ Helper::amountFormatDecimal($user->wallet)}}</td>
                     <td data-label="{{ __('general.posts') }}">{{ $user->updates()->count() }}</td>
                     <td data-label="{{ __('admin.date') }}">{{ Helper::formatDate($user->date) }}</td>
                     <td data-label="{{ __('general.last_login') }}">{{ Helper::formatDate($user->last_seen) }}</td>
                     <td data-label="IP">{{ $user->ip ? $user->ip : __('general.no_available') }}</td>
                     <td data-label="{{ __('admin.role') }}">
						...
						@endphp
		                        ...
		                        <td data-label="{{ __('admin.verified') }}"><span class="rounded-pill badge bg-{{$verified}}">{{ $_verified }}</span></td>
                    ...
                    @endphp
                     ...
                     <td data-label="{{ __('admin.status') }}"><span class="rounded-pill badge bg-{{$mode}}">{{ $_status }}</span></td>
                     <td data-label="{{ __('admin.actions') }}">
