               ...
               @if ($data->count() !=  0)
                 ...
                 @foreach ($data as $report)
					<tr>
						...
						<td data-label="{{ __('admin.report_by') }}"><a href="{{ url($report->user()->username) }}" target="_blank">{{ $report->user()->name }} <i class="fa fa-external-link-square-alt"></i></a></td>

						<td data-label="{{ __('admin.reported') }}">
						@switch($report->type)
							@case('user')
					...
					<td data-label="{{ __('general.message') }}">
						{{ $report->message ?? '--' }}
					</td>
					...
					@endphp
						...
						<td data-label="{{ __('admin.reason') }}">{{ $reason }}</td>

						<td data-label="{{ __('admin.date') }}">{{ Helper::formatDate($report->created_at) }}</td>
