  ...
  <div class="card w-100 rounded-0 h-100 border-top-0">
    <div class="card-header bg-white pt-4">
      <div class="media">
        ...
        <a href="javascript:void(0);" class="f-size-20 text-warning float-right mr-3 text-decoration-none @if (Helper::isOnline($user->id)) requestLivePrivateModal @else buttonDisabled @endif" @if (Helper::isOnline($user->id)) data-toggle="tooltip" data-placement="bottom" title="{{ __('general.request_private_live_stream') }}" @endif role="button">
          ...
          @if ($subscribedToYourContent || $subscribedToMyContent || auth()->user()->isSuperAdmin() || $user->isSuperAdmin())
            ...
            <form action="{{url('message/send')}}" method="post" accept-charset="UTF-8" id="formSendMsg" enctype="multipart/form-data">
              ...
              <div class="d-flex w-100 position-relative">
                <div class="flex-grow-1 mr-2 position-relative">
                  <div>
                ...
                <div class="align-self-center d-flex">
                  ...
                  <button type="button" id="recordAudio" class="btn btn-upload btn-tooltip e-none mr-2 @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill" data-toggle="tooltip" data-placement="top" title="{{__('general.record_audio')}}">
                    <i class="bi-mic f-size-25"></i>
                  </button>
                  <button type="button" id="stopAudio" class="btn btn-upload btn-tooltip e-none mr-2 text-danger rounded-pill display-none" data-toggle="tooltip" data-placement="top" title="{{__('general.stop_recording')}}">
                    <i class="bi-stop-circle f-size-25"></i>
                  </button>
                  
                  <button type="submit" id="button-reply-msg" disabled data-send="{{ __('auth.send') }}" data-wait="{{ __('general.send_wait') }}" class="btn btn-sm btn-primary rounded-pill e-none">
                    <i class="far fa-paper-plane mr-1"></i> {{ __('auth.send') }}
                  </button>
                </div>
              </div>
...
<script src="{{ asset('public/js/audio-recorder.js') }}?v={{$settings->version}}"></script>
<script src="{{ asset('public/js/video-call.js') }}?v={{$settings->version}}"></script>
