...
use App\Http\Controllers\Admin\InitialPostController;
...
Route::get('blog/post/{id}/{slug?}', [BlogController::class, 'post'])->name('seo.blog.post.{id}.{slug?}');
...
Route::group(['middleware' => 'guest'], function () {
	Route::get('oauth/{provider}', [SocialAuthController::class, 'redirect'])->where('provider', '(facebook|google|twitter|x)$');
	Route::get('oauth/{provider}/callback', [SocialAuthController::class, 'callback'])->where('provider', '(facebook|google|twitter|x)$');
});//<--- End Group guest
...
//    Route::post('parcerias/enviar', function () {
//        $usuario = auth()->user();
//        $parcerias = App\Models\Parceria::all();
//        dd($request->all()); // Muestra los datos del request
//        dd($usuario); // Muestra los datos del usuario autenticado
//    })->middleware('auth');

	// Crear una parceria
	Route::post('parcerias/enviar', [ParceriaController::class, 'store'])->name('parcerias.store');
	// Agregar historial a una parceria
	Route::post('parcerias/{parceria}/historial', [ParceriaController::class, 'addHistory'])->name('parcerias.addHistory');
	// Mostrar parceria con historial
	Route::get('parceria', [ParceriaController::class, 'index'])->name('parceria.index');
	Route::get('parcerias/{id}', [ParceriaController::class, 'show'])->name('parcerias.show');
	// routes/web.php
	Route::get('/parceria/{id}/mensajes', [ParceriaController::class, 'mensajes'])->name('parceria.mensajes');
	Route::post('/parcerias/{parceria}/mensajes/enviar', [ParceriaController::class, 'enviar'])
		->name('parcerias.mensajes.enviar');
...
Route::group(['middleware' => 'private.content'], function () {
	...
	Route::get('shop/product/{id}/{slug?}', [ProductsController::class, 'show'])->name('seo.shop.product.{id}.{slug?}');

	// Creators
	Route::get('creators/{type?}', [HomeController::class, 'creators']);

	// Category
	Route::get('category/{slug}/{type?}', [HomeController::class, 'category'])->name('seo.category.{slug}.{type?}');

	// Profile User
	Route::get('{slug}', [UserController::class, 'profile'])->where('slug', '[A-Za-z0-9\_-]+')->name('profile.alt1');
	Route::get('{slug}/{media}', [UserController::class, 'profile'])->where('media', '(photos|videos|audio|shop|files|epub)$')->name('profile.{slug}.{media}');

	// Profile User
	Route::get('{slug}/post/{id}', [UserController::class, 'postDetail'])->where('slug', '[A-Za-z0-9\_-]+')->name('profile.{slug}.post.{id}');
...
Route::group(['middleware' => 'role'], function () {
	...
	Route::prefix('panel/admin')->group(function () {
		...
		// Settings
		Route::get('/settings', [AdminController::class, 'settings'])->name('general.settings');
		Route::post('/settings', [AdminController::class, 'saveSettings']);

		// Limits
		Route::get('/settings/limits', [AdminController::class, 'settingsLimits'])->name('general.settings.limits');
		Route::post('/settings/limits', [AdminController::class, 'saveSettingsLimits']);

		// Video Encoding
		Route::view('/video/encoding', 'admin.video_encoding')->name('general.video.encoding');
		...
		//Withdrawals
		Route::get('/withdrawals', [AdminController::class, 'withdrawals'])->name('withdrawals.withdrawals');
		Route::get('/withdrawal/{id}', [AdminController::class, 'withdrawalsView'])->name('withdrawals.withdrawal.{id}');
		...
		// Members
		Route::get('/members', [AdminController::class, 'index'])->name('members.members');


		// EDIT MEMBER
		Route::get('/members/edit/{id}', [AdminController::class, 'edit'])->name('members.members.edit.{id}');
		...
		Route::get('top-creadores', [HighlightController::class, 'index'])->name('admin.top-creadores');
		Route::post('top-creadores/add', [HighlightController::class, 'add'])->name('admin.top-creadores.add');
		Route::post('top-creadores/remove/{user}', [HighlightController::class, 'remove'])->name('admin.top-creadores.remove');

		// Initial Posts Admin
		Route::get('initial-posts', [InitialPostController::class, 'index'])->name('admin.initial-posts.index');
		Route::get('initial-posts/create', [InitialPostController::class, 'create'])->name('admin.initial-posts.create');
		Route::post('initial-posts', [InitialPostController::class, 'store'])->name('admin.initial-posts.store');
		Route::get('initial-posts/{id}/edit', [InitialPostController::class, 'edit'])->name('admin.initial-posts.edit');
		Route::post('initial-posts/{id}', [InitialPostController::class, 'update'])->name('admin.initial-posts.update');
		Route::delete('initial-posts/{id}', [InitialPostController::class, 'destroy'])->name('admin.initial-posts.destroy');
		// AJAX preview
		Route::get('initial-posts/preview/{id}', [InitialPostController::class, 'preview'])->name('admin.initial-posts.preview');
		


		// Pages
		Route::get('/pages', [PagesController::class, 'index'])->name('pages.pages');

		// ADD NEW PAGES
		Route::get('/pages/create', [PagesController::class, 'create'])->name('pages.pages.create');

		// ADD NEW PAGES POST
		Route::post('/pages/create', [PagesController::class, 'store']);

		// EDIT PAGES
		Route::get('/pages/edit/{id}', [PagesController::class, 'edit'])->name('pages.pages.edit.{id}');
		...
		// Payments Settings
		Route::get('/payments', [AdminController::class, 'payments'])->name('payments.payments');
		Route::post('/payments', [AdminController::class, 'savePayments']);

		Route::get('/payments/{id}', [AdminController::class, 'paymentsGateways'])->name('payments.payments.{id}');
		...
		// Categories
		Route::get('/categories', [AdminController::class, 'categories'])->name('categories.categories');
		Route::get('/categories/add', [AdminController::class, 'addCategories'])->name('categories.categories.add');
		Route::post('/categories/add', [AdminController::class, 'storeCategories']);
		Route::get('/categories/edit/{id}', [AdminController::class, 'editCategories'])->name('categories.categories.edit.{id}');
		...
		//***** Languages
		Route::get('/languages', [LangController::class, 'index'])->name('languages.languages');

		// ADD NEW
		Route::get('/languages/create', [LangController::class, 'create'])->name('languages.languages.create');

		// ADD NEW POST
		Route::post('/languages/create', [LangController::class, 'store']);

		// EDIT LANG
		Route::get('/languages/edit/{id}', [LangController::class, 'edit'])->name('languages.languages.edit.{id}');
		...
		// Maintenance mode
		Route::view('/maintenance/mode', 'admin.maintenance_mode')->name('maintenance_mode.maintenance.mode');
		Route::post('/maintenance/mode', [AdminController::class, 'maintenanceMode']);

		// Clear Cache
		Route::get('/clear-cache', [AdminController::class, 'clearCache'])->name('maintenance_mode.clear_cache');

		// Blog
		Route::get('/blog', [AdminController::class, 'blog'])->name('blog.blog');
		Route::post('/blog/delete/{id}', [AdminController::class, 'deleteBlog']);

		// Add Blog Post
		Route::view('/blog/create', 'admin.create-blog')->name('blog.blog.create');
		Route::post('/blog/create', [AdminController::class, 'createBlogStore']);

		// Edit Blog Post
		Route::get('/blog/{id}', [AdminController::class, 'editBlog'])->name('blog.blog.{id}');
		Route::post('/blog/update', [AdminController::class, 'updateBlog']);

		// Resend confirmation email
		Route::get('/resend/email/{id}', [AdminController::class, 'resendConfirmationEmail'])->name('members.resend.email.{id}');

		// Deposits
		Route::get('/deposits', [AdminController::class, 'deposits'])->name('deposits.deposits');
		Route::get('/deposits/{id}', [AdminController::class, 'depositsView'])->name('deposits.deposits.{id}');
		...
		Route::get('/members/roles-and-permissions/{id}', [AdminController::class, 'roleAndPermissions'])->name('members.members.roles_and_permissions.{id}');
		Route::post('/members/roles-and-permissions/{id}', [AdminController::class, 'storeRoleAndPermissions']);

		// Shop Categories
		Route::get('/shop-categories', [AdminController::class, 'shopCategories'])->name('shop_categories.shop_categories');
		Route::get('/shop-categories/add', [AdminController::class, 'addShopCategories'])->name('shop_categories.shop_categories.add');
		Route::post('/shop-categories/add', [AdminController::class, 'storeShopCategories']);
		Route::get('/shop-categories/edit/{id}', [AdminController::class, 'editShopCategories'])->name('shop_categories.shop_categories.edit.{id}');
		...
		Route::get('/tax-rates', [TaxRatesController::class, 'show'])->name('tax.tax_rates');
		Route::view('/tax-rates/add', 'admin.add-tax')->name('tax.tax_rates.add');
		Route::post('/tax-rates/add', [TaxRatesController::class, 'store']);
		Route::get('/tax-rates/edit/{id}', [TaxRatesController::class, 'edit'])->name('tax.tax_rates.edit.{id}');
		Route::post('/tax-rates/update', [TaxRatesController::class, 'update']);
		Route::post('/ajax/states', [TaxRatesController::class, 'getStates']);

		Route::get('/countries', [CountriesStatesController::class, 'countries'])->name('countries_states.countries');
		Route::view('/countries/add', 'admin.add-country')->name('countries_states.countries.add');
		Route::post('/countries/add', [CountriesStatesController::class, 'addCountry']);
		Route::get('/countries/edit/{id}', [CountriesStatesController::class, 'editCountry'])->name('countries_states.countries.edit.{id}');
		...
		Route::get('/states', [CountriesStatesController::class, 'states'])->name('countries_states.states');
		Route::view('/states/add', 'admin.add-state')->name('countries_states.states.add');
		Route::post('/states/add', [CountriesStatesController::class, 'addState']);
		Route::get('/states/edit/{id}', [CountriesStatesController::class, 'editState'])->name('countries_states.states.edit.{id}');
		...
		// Stories
		Route::view('/stories/settings', 'admin.stories-settings')->name('stories.stories.settings');
		Route::post('/stories/settings', [AdminController::class, 'saveStoriesSettings']);

		// Stories Posts
		Route::get('/stories/posts', [AdminController::class, 'storiesPosts'])->name('stories.stories.posts');
		Route::post('/stories/posts/delete/{id}', [AdminController::class, 'deleteStory']);

		// Stories Backgrounds
		Route::get('/stories/backgrounds', [AdminController::class, 'storiesBackgrounds'])->name('stories.stories.backgrounds');
		...
		// Stories Fonts
		Route::get('/stories/fonts', [AdminController::class, 'storiesFonts'])->name('stories.stories.fonts');
		...
		// Comments on Posts
		Route::get('/comments', [AdminController::class, 'comments'])->name('comments_replies.comments');
		Route::post('/comments/delete/{id}', [AdminController::class, 'deleteComment']);

		// Replies comments on posts
		Route::get('/replies', [AdminController::class, 'replies'])->name('comments_replies.replies');
		...
		// Advertising
		Route::get('/advertising', [AdvertisingController::class, 'show'])->name('advertising.advertising');
		Route::view('/advertising/add', 'admin.add-advertising')->name('advertising.advertising.add');
		Route::post('/advertising/store', [AdvertisingController::class, 'store'])->name('advertising.store');
		Route::get('/advertising/edit/{ad}', [AdvertisingController::class, 'edit'])->name('advertising.advertising.edit.{ad}');
